...
INGRESS_NAMESPACE="ingress-clouddev"
NAMESPACE="clouddev"
SERVICE_NAME="webapp"
...
echo "Deploy ingress for service ${SERVICE_NAME} on AKS "
...
kubectl delete -f "${TEMP_DIR}/webapp-ingress.yml"  --namespace $NAMESPACE 2>/dev/null || true 
echo "kubectl apply -f ${TEMP_DIR}/webapp-ingress.yml  --namespace $NAMESPACE"
kubectl apply -f "${TEMP_DIR}/webapp-ingress.yml"  --namespace $NAMESPACE

echo "Wait 30 seconds"
sleep 30

echo "Test the service on AKS: http://${FQDN}/${SERVICE_NAME}"
RESULT=$(curl -i -s -X GET http://${FQDN}/${SERVICE_NAME} | grep  "<title>Welcome") || true
echo "RESULT: ${RESULT}"
if [ -z "${RESULT}" ]
then
  echo "Ingress Deployment failed: http://${FQDN}/${SERVICE_NAME}"
else
  echo "Ingress Deployment successful: http://${FQDN}/${SERVICE_NAME}"
fi
...
echo "kubectl delete -f ${TEMP_DIR}/-ingress.yml --namespace $NAMESPACE"
